...
WhatsApp Cloud API onboarding is in progress without changing that working flow. The hosted `whatsapp-webhook-v1` Supabase Edge Function verifies Meta's callback token, checks every delivery against the app-secret HMAC signature, limits request size, and stores each signed request once in a staff-only webhook inbox. A separate staff-only management function installed the `messages` subscription using the server-held permanent access token, and a real sandbox reply reached that inbox. Automated outbound sends remain disabled until the real business number, approved templates, an idempotent outbox, and end-to-end delivery tests are complete.
