...
1. The repository is linked to the healthy Supabase Free project `Road` (`abpmzqtbllszqqetuubp`) in `eu-west-1`; all 25 migrations are applied.
...
| Purpose | Template | Meta ID | Language | Status at 00:23 on 12 September |
| --- | --- | --- | --- | --- |
...
3. The backend supports structured **Laus** and **Ekki laus** replies. The next interface pass must return that candidate to the dispatcher for deliberate assignment; it must never silently replace the existing manual assignment decision.
4. API sends are correlated to the job and recipient using Meta's WhatsApp message ID. Webhooks update the explicit `queued`/`accepted`/`sent`/`delivered`/`read`/`failed` lifecycle; the next interface pass must add those verified facts to the staff timeline.
...
The generic send/status/reply/idempotency backend foundation is complete and live-tested, and the operational interface wiring plus local automatic-to-manual fallback pass are complete. The production slice remains open until template approval, operational sandbox send/reply/failure tests, opt-out handling, staff-timeline status/reply rendering, and a real-number phone pass prove customer delivery, driver replies, assignment delivery, and one-time link use.
...
- `npm test` passed all 162 tests across 32 Vitest files, including operational template definitions, URL-safe one-time driver button codes, authenticated Edge Function invocation, outbound template validation, safe provider-failure classification, webhook normalization, Meta-subscription handling, and direct authenticated Storage URL loading.
- `npx supabase test db` passed all 253 assertions across eleven pgTAP files, including outbound idempotency, status/reply correlation, early-webhook backfill, ambiguous-retry prevention, driver denial, the raw webhook inbox, Storage object authorization, and the restricted Realtime publication.
...
3. Wait for Meta to approve the three submitted templates, then run the customer-intake, driver-availability reply, assignment/access, provider-failure, and manual-fallback paths through the hosted interface and sandbox phone.
