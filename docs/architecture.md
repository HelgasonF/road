...
The `whatsapp-management-v1` Edge Function is the administrative side of that boundary. It requires an authenticated Supabase session, verifies `is_staff()` before reading any protected credential, and can install only the WhatsApp Business Account `messages` subscription. Meta app secret, webhook token, permanent access token, and WABA identifiers stay in Supabase secrets; the response exposes only success booleans. Runtime message sending will use a separate narrow outbox contract rather than expanding this configuration endpoint.
