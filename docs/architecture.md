...
- `whatsapp_webhook_events` keeps the deduplicated raw envelope for every Meta-signed callback. `whatsapp_delivery_events` and `whatsapp_inbound_messages` hold the normalized status and reply records used by Vegstoð.
- `whatsapp_outbound_messages` is the idempotent Cloud API send ledger. It stores business references, recipient, template metadata, state, and a payload fingerprint; template parameters and bearer links are never retained in plaintext.
...
The `driver-access-v1` Supabase Edge Function performs driver Auth administration. It first checks `is_staff()` with a caller-scoped client, then uses a service-role client only for `auth.admin` calls; its versioned response returns a first-party Vegstoð path and never an internal Auth address or Supabase verification URL. Vercel's server-only Supabase secret remains limited to account-free customer-link and Storage operations. Those calls validate the hashed, unexpired, unrevoked token before returning job fields or creating signed upload/download access; the secret is never serialized to a Client Component.

The public `whatsapp-webhook-v1` Edge Function is the inbound trust boundary for Meta. Its Supabase JWT check is disabled because Meta cannot supply a user session, so the function validates the setup token for GET challenges and the app-secret `X-Hub-Signature-256` HMAC over the raw POST body. One database RPC atomically retains the deduplicated envelope, normalizes `sent`/`delivered`/`read`/`failed` updates, advances a linked outbound record, and stores inbound replies. A **Laus** or **Ekki laus** reply is only a dispatcher-visible classification; it never assigns a job or changes an operator automatically. RLS permits staff reads and denies drivers and anonymous users.

The `whatsapp-management-v1` Edge Function is the administrative side of that boundary. It requires an authenticated Supabase session and verifies `is_staff()` before reading any protected credential. It can install the sandbox WhatsApp Business Account `messages` subscription or perform a read-only production-account inspection that returns only selected phone-status fields and subscribed app IDs. Operational templates are reviewed and submitted manually in WhatsApp Manager; the function cannot create them. The separate `whatsapp-send-v1` function accepts only authenticated staff template requests, validates job/operator/link references in PostgreSQL, claims one idempotency key atomically, and calls the configured Phone Number ID. Definite provider rejections may be retried with the same payload; timeouts, 5xx responses, and malformed success responses become `delivery_unknown` because an automatic retry could duplicate a message. Signed `STOP`/`HÆTTA` replies record an explicit opt-out and prevent later sends; a newer signed `START`/`BYRJA` reply re-enables them. Meta credentials remain only in Supabase secrets.

The photo bucket has no anonymous read policy. Its authenticated `storage.objects` policy requires an uploaded `job_photos` metadata row with the same exact object path. That metadata subquery retains its existing RLS rule, so staff see authorized photos and a driver sees only photos for a current assignment. Their browsers request five-minute signed URLs directly from Storage. The account-free customer photo route remains server-side because customer authorization is based on a hashed intake token rather than a Supabase session.
...
Driver access is passwordless and delivered through a WhatsApp template with a dynamic secure URL button. The staff-only Server Action calls the versioned Auth Edge Function with the staff member's Supabase access token. The function repeats staff authorization before generating an Auth link for an internal, non-routable driver identifier. Vegstoð converts that first-party `/driver/access` path into a URL-safe button suffix without storing it in the outbound ledger. Opening the link displays a confirmation button instead of immediately consuming the token, so link-preview requests do not sign in or invalidate the link. The confirmed one-time token creates the normal Supabase cookie session, after which all driver reads and mutations remain constrained by the existing driver RLS policies. The internal Auth identifier is never requested from or shown to the driver.

`jobs`, `job_assignments`, and `operators` are the only application tables in the `supabase_realtime` publication. Existing web screens still refresh through Server Actions and `router.refresh()`; the publication is an inert backend capability until a client subscribes. Financial, contact, customer-token, and photo-metadata tables are intentionally excluded.
...
The staff-only timeline route normalizes existing source-of-truth rows rather than copying them into a second generic activity table. Job creation, status history, customer-link lifecycle, uploaded photos, assignment lifecycle, driver contact attempts, WhatsApp acceptance/delivery/read/failure events, correlated replies, and billing events remain owned by their domain tables and are merged newest-first at read time. The candidate list also shows the latest correlated **Available**/**Unavailable** reply without changing the manual assignment decision. Drivers have no RLS policy for the contact, messaging, preference, or timeline data.
...
- Driver accounts are created from dispatcher-generated passwordless links sent to the registered phone through a WhatsApp Cloud API template, with a prepared manual WhatsApp fallback, and linked one-to-one to existing operators; drivers do not provide an email or password, and disabling access never deletes the operator or operational history.
- Driver contact uses Cloud API templates. Candidate messages are built from a narrow operational summary that cannot contain customer contact details, notes, photos, or intake tokens; house numbers and raw map coordinates are removed from the pre-assignment area label. The post-assignment template receives a fresh, expiring one-time driver access suffix for any non-disabled assigned operator. Prepared `wa.me` and phone actions remain available as fallbacks.
- Customer-link delivery uses the same Cloud API boundary: the default **+** flow asks only for the phone number, atomically creates a pending job and raw intake token, and sends the customer template with a dynamic secure URL button. Staff can switch to the complete job form. Pending jobs cannot enter map matching or assignment. The form data and photos are submitted to Vegstoð and private Supabase Storage, not sent through WhatsApp. The raw URL is not recoverable from the stored hash, so resending later requires rotating to a new link. A failed automatic send keeps that freshly generated URL available through the manual `wa.me` fallback.
