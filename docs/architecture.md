...
The public `whatsapp-webhook-v1` Edge Function is the inbound trust boundary for Meta. Its Supabase JWT check is disabled because Meta cannot supply a user session, so the function validates the setup token for GET challenges and the app-secret `X-Hub-Signature-256` HMAC over the raw POST body. Valid envelopes enter an append-only, deduplicated `whatsapp_webhook_events` inbox through the service role. RLS permits staff audit reads and denies drivers and anonymous users. No webhook payload changes job or assignment state directly; later processing must correlate and validate a recognized message before invoking a narrow domain operation.

The `whatsapp-management-v1` Edge Function is the administrative side of that boundary. It requires an authenticated Supabase session and verifies `is_staff()` before reading any protected credential. It can install the sandbox WhatsApp Business Account `messages` subscription or perform a read-only production-account inspection that returns only selected phone-status fields and subscribed app IDs. Meta app secret, webhook token, permanent access token, and WABA identifiers stay in Supabase secrets. Runtime message sending will use a separate narrow outbox contract rather than expanding this configuration endpoint.
...
- Driver contact remains manual in the MVP: normal `wa.me` links open WhatsApp or WhatsApp Web and the dispatcher presses Send. Candidate messages are built from a narrow operational summary that cannot contain customer contact details, notes, photos, or intake tokens; house numbers and raw map coordinates are removed from the pre-assignment area label. The post-assignment variant generates a fresh, expiring one-time driver access URL for any non-disabled assigned operator. Contact-link openings are audited, but an external send or connected call cannot be verified. The Cloud API webhook inbox, `messages` subscription, and a real sandbox inbound delivery are deployed as production groundwork; they do not replace the operational send path until outbound processing and the real-number phone tests pass.
