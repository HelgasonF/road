...
- Driver contact remains manual in the MVP: normal `wa.me` links open WhatsApp or WhatsApp Web and the dispatcher presses Send. Candidate messages are built from a narrow operational summary that cannot contain customer contact details, notes, photos, or intake tokens; house numbers and raw map coordinates are removed from the pre-assignment area label. The post-assignment variant generates a fresh, expiring one-time driver access URL for any non-disabled assigned operator. Contact-link openings are audited, but an external send or connected call cannot be verified. The Cloud API webhook inbox is deployed as production groundwork; it does not replace the operational send path until the remaining Meta configuration and end-to-end tests pass.
