...
The operational interface, dynamic secure-link buttons, candidate reply display, staff delivery/reply timeline, and signed STOP/START preference guard were subsequently implemented and deployed. Local and hosted fallback passes succeeded while the first three operational templates remained pending Meta review. Manual review later found those API-created templates were wrong, and the user deleted them. Supabase now names manually entered replacements, and the API template-creation action has been removed. The disposable hosted failure and preference-check records were removed. The evidence boundary above remains the boundary of the 11 September physical-phone pass itself; the next phone pass starts after the replacement templates are verified and approved. See [`whatsapp-production-handoff-2026-09-12.md`](whatsapp-production-handoff-2026-09-12.md).
